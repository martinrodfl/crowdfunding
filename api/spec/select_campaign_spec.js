describe('/select_campaign', function () {
  ...
  it('fails to find campaign', function(done){
    request({
      port: port,
      path: '/select_campaign',
      body: JSON.stringify({ 
  ...
  it('finds, select and present campaign', function(done){
    ...
    request({
      ...
      .then(function(response){
        ...
        db.collection('campaigns').find().toArray()
          .then(function(campaignsDb){
            expect(campaigns[0].active).toEqual(campaignsDb[0].active);
            expect(campaigns[1].active).toEqual(!campaignsDb[1].active);
            expect(campaigns[2].active).toEqual(!campaignsDb[2].active);
            expect(campaigns[3].active).toEqual(campaignsDb[3].active);
            done();
          })
      })
  })
